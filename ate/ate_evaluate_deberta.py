...
def predict(model, tokenizer, dataset, device, batch_size=16):   # smaller batch for DeBERTa
    model.eval()
    y_true, y_pred = [], []
...
def main():
    ...
    print(f"\n{'='*40}")
    print(f"ATE Evaluation Results (DeBERTa-v3-base)")
    print(f"{'='*40}")
    ...
    results  = {"model": args.model_dir, "precision": p, "recall": r, "f1": f1, "report": report}
